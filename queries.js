...
module.exports = {
  ...
  get_evaluations_from_course: get_evaluations_from_course,
};

let queries_sql = {

  get_course_info: "SELECT * FROM COURSE WHERE id = ?;",
  get_evaluations_from_course: "SELECT         STUDENT_IN_COURSE.student AS student,        EVALUATION.id AS eval_id,        EVALUATION.name AS eval_name,        value   FROM STUDENT_IN_COURSE        LEFT JOIN        EVALUATION USING (            course        )        LEFT JOIN        GRADE ON (GRADE.evaluation = EVALUATION.id AND                   STUDENT_IN_COURSE.student = GRADE.student)         JOIN        COURSE ON (STUDENT_IN_COURSE.course = COURSE.id)        WHERE course = ?; ",
  ...
  get_timeline_info_on_course: "SELECT student, act_id, start, [end], s_start FROM ( SELECT STUDENT_IN_COURSE.student, QUIZ.id AS act_id, time_open AS start, time_close AS [end], finish AS s_start, course FROM STUDENT_IN_COURSE LEFT JOIN QUIZ USING ( course ) LEFT JOIN ATTEMPT ON (ATTEMPT.student = STUDENT_IN_COURSE.student AND ATTEMPT.quiz = QUIZ.id) UNION ALL SELECT STUDENT_IN_COURSE.student, ASSIGN.id AS act_id, time_open AS start, time_close AS [end], created AS s_start, course FROM STUDENT_IN_COURSE LEFT JOIN ASSIGN USING ( course ) LEFT JOIN SUBMISSION ON (SUBMISSION.student = STUDENT_IN_COURSE.student AND SUBMISSION.assign = ASSIGN.id) UNION ALL SELECT STUDENT_IN_COURSE.student, FORUM.id AS act_id, time_open AS start, time_close AS [end], created AS s_start, course FROM STUDENT_IN_COURSE LEFT JOIN FORUM USING ( course ) LEFT JOIN POST ON (POST.student = STUDENT_IN_COURSE.student AND POST.forum = FORUM.id) ) AS tab1 WHERE course = ?"
};